...
__global__ void updateCentroids(song *data_d, int *cluster_assignment_d, song *centroids_d, int *cluster_sizes_d)
{
    ...
    if (thread_id == 0)
    {
        ...
        for (int j = 0; j < N; ++j)
        {
            int cluster_id = cluster_assignment_d[j];
            data_block_sums[cluster_id].danceability += data_d[j].danceability;
            data_block_sums[cluster_id].energy += data_d[j].energy;
            data_block_sums[cluster_id].loudness += data_d[j].loudness;
            data_block_sums[cluster_id].speechiness += data_d[j].speechiness;
            data_block_sums[cluster_id].acousticness += data_d[j].acousticness;
            data_block_sums[cluster_id].instrumental += data_d[j].instrumental;
            data_block_sums[cluster_id].liveness += data_d[j].liveness;
            data_block_sums[cluster_id].valence += data_d[j].valence;
            data_block_sums[cluster_id].tempo += data_d[j].tempo;
